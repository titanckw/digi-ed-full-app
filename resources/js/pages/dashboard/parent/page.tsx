...
export default function ParentDashboard() {
  ...
  const handleMonitorSession = (sessionId: number) => {
    ...
    setTimeout(() => {
      alert(`Live monitoring started for session ${sessionId}. You can now view the session in real-time.`)
    }, 500)

    // Auto-stop monitoring after demo period
    setTimeout(() => {
      setIsMonitoring(false)
      alert("Monitoring session ended. Session recording has been saved.")
    }, 5000)
  }
  ...
  const handleEmergencyStop = () => {
    if (confirm("Are you sure you want to immediately end the current session? This action cannot be undone.")) {
      setIsMonitoring(false)
      alert(
      ...
      <div className="container mx-auto px-4 py-8">
            ...
            {children.map((child) => (
              <Card
                ...
                className={`cursor-pointer transition-all duration-200 ${
                  selectedChild === child.id ? "ring-2 ring-primary bg-primary/5" : "hover:shadow-md"
                }`}
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-primary">{currentChild.totalSessions}</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-secondary">{currentChild.hoursLearned}</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-primary">{currentChild.activeTutors}</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-yellow-600">{currentChild.averageRating}</p>
                </div>
        ...
        <Tabs defaultValue="monitoring" className="space-y-6">
          ...
          <TabsContent value="monitoring" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Monitor ongoing sessions in real-time with safety controls</CardDescription>
              </CardHeader>
              <CardContent>
                {isMonitoring ? (
                    ...
                    <div className="bg-accent/10 border border-accent/20 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="font-medium text-accent">Live Session Active</span>
                        </div>
                        <Badge className="bg-accent/10 text-accent border-accent/20">Emma with Dr. Sarah Johnson</Badge>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div className="text-center">
                          <p className="text-sm text-gray-600">Session Time</p>
                          <p className="text-lg font-bold">23:45</p>
                        </div>
                        <div className="text-center">
                          <p className="text-sm text-gray-600">Focus Level</p>
                          <p className="text-lg font-bold text-accent">Excellent</p>
                        </div>
                        <div className="text-center">
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-600 mb-4">No active sessions to monitor</p>
                    <p className="text-sm text-gray-500">
                      When your child starts a session, you'll be able to monitor it here
                    </p>
                  </div>
                )}
              </CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Sessions you can monitor today</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {upcomingSessions.map((session) => (
                    <div key={session.id} className="flex items-center justify-between p-4 border rounded-lg">
                      ...
                      <div className="flex space-x-2">
                        <Button variant="outline" size="sm" onClick={() => handleMonitorSession(session.id)}>
                          <Eye className="w-4 h-4 mr-2" />
                          Monitor
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => handleContactTutor(session.tutor)}>
                          <MessageCircle className="w-4 h-4 mr-2" />
                          Contact
                        </Button>
                      </div>
          ...
          <TabsContent value="sessions" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Detailed session history and recordings</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    <div key={session.id} className="border rounded-lg p-4 space-y-4">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            with {session.tutor} • {session.date}
                          </p>
                        </div>
                        <div className="flex items-center space-x-2">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center space-x-2">
                            <Progress value={session.behaviorScore} className="flex-1" />
                            <span className="text-sm font-medium">{session.behaviorScore}%</span>
                          </div>
                        </div>
                        <div className="space-y-2">
                          ...
                          <Badge
                            className={`${
                              session.focusLevel === "Excellent"
                                ? "bg-accent/10 text-accent border-accent/20"
                                : session.focusLevel === "Good"
                      ...
                      <div className="flex space-x-2 pt-2 border-t">
                        <Button variant="outline" size="sm" onClick={() => handleViewRecording(session.id)}>
          ...
          <TabsContent value="progress" className="space-y-6">
            <Card>
              <CardHeader>
                <CardTitle>Learning Progress - {currentChild.name}</CardTitle>
                <CardDescription>Track academic improvement across subjects</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-center">
                    ...
                    <p className="text-sm text-accent mt-2">↑ 15% from last week</p>
                  </div>
                </CardContent>
              </Card>
                    ...
                    <div className="flex justify-center mb-2">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`w-5 h-5 ${
                            i < Math.floor(4.8) ? "text-yellow-400 fill-current" : "text-gray-300"
                          }`}
                        />
                      ))}
                    </div>
                    <p className="text-sm text-gray-600">Based on 24 sessions</p>
                  </div>
                </CardContent>
              </Card>
            </div>
          ...
          <TabsContent value="safety" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Monitor safety features and alerts</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex items-center space-x-3 p-3 bg-accent/10 rounded-lg">
                      ...
                      <div>
                        <p className="font-medium text-accent">Background Checks</p>
                        <p className="text-sm text-accent/80">All tutors verified</p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-3 p-3 bg-accent/10 rounded-lg">
                      <CheckCircle className="w-5 h-5 text-accent" />
                      <div>
                        <p className="font-medium text-accent">Session Recording</p>
                        <p className="text-sm text-accent/80">Always enabled</p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-3 p-3 bg-accent/10 rounded-lg">
                      <CheckCircle className="w-5 h-5 text-accent" />
                      <div>
                        <p className="font-medium text-accent">Live Monitoring</p>
                        <p className="text-sm text-accent/80">Available 24/7</p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-3 p-3 bg-accent/10 rounded-lg">
                      <CheckCircle className="w-5 h-5 text-accent" />
                      <div>
                        <p className="font-medium text-accent">Emergency Controls</p>
                        <p className="text-sm text-accent/80">Instant session stop</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>System notifications and safety updates</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {safetyAlerts.map((alert) => (
                    ...
                    >
                      {alert.type === "success" && <CheckCircle className="w-5 h-5 text-accent mt-0.5" />}
                      {alert.type === "warning" && <AlertTriangle className="w-5 h-5 text-yellow-600 mt-0.5" />}
                      {alert.type === "info" && <Shield className="w-5 h-5 text-blue-600 mt-0.5" />}
          ...
          <TabsContent value="reports" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Comprehensive learning and behavior reports</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Button
